...
public class Trench_Placement : MonoBehaviour {
				...
				if (Physics.Raycast(ray,out hit, Mathf.Infinity, GroundMask))	// if the player has touched the ground
				{
					...
					for (int n = 0; n < trenches.Count; n++)
					{
						...
						else
						{
							...
							Trench = current_Trench.gameObject;
					...
					if (trenches.Count == 0)
					{
						trenches.Add(current_Trench.gameObject);
					}
				}			
			}
		}
	...
	public void HasTrenchPlacementFinished()
	{
		if (Input.GetMouseButtonDown(0))
		{
			...
			{
				Destroy(current_Trench.gameObject);
				SettingTrenches = false;
				Destroy (Tick_Clone);
				Destroy (Cross_Clone);
				Destroy (Moving_Building_Text_Clone);
	...
	{
		checkForCollisions = current_Trench.GetComponentInChildren<CheckForCollisions> ();
	...
	public void SetTrench(GameObject b)
	{
		...
		current_Trench = ((GameObject)Instantiate(b)).transform;
		Vector3 currentGrid = grid.ActiveGrid.position;
